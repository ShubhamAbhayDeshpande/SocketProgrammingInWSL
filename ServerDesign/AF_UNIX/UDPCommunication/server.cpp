...
#define SOCKET_NAME "/tmp/UDPServerSocket"
...
int main()
{
    ...
    connectionSocket = socket(AF_UNIX, SOCK_DGRAM, 0);
    ...
    */
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            ret = recvfrom(connectionSocket, buffer, sizeof(int), 0,
                       (struct sockaddr *)&name, &client_len);

            // If the read() fails, return error and end program. Note here, that read can return 0. Hence, the error is called at -1.
            if (ret <= 0)
            {
                cerr<<"recvfrom() function call failed. "<<endl;
                return(EXIT_FAILURE);
            }
        ...
        ret = ret = sendto(connectionSocket, &result, sizeof(result), 0,
                         (const struct sockaddr *)&name, client_len);
